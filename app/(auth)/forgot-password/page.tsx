...
export default function ForgotPassword() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      // After "sending" code, navigate to verify page
      router.push(`/verify-forgot-password?email=${encodeURIComponent(email)}`);
    }, 1500);
  };
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          <div className="text-center lg:text-left space-y-4">
            <Link href="/login" className="hover:underline flex items-center gap-2">
              <img src="/k-3.png" alt="back-arrow" className="w-5" />
              Back to sign in
            </Link>

            <img src="/k-5.png" alt="logo" className="w-10 mt-7 mb-7 mx-auto lg:mx-0" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="email" className="text-foreground mb-3 mt-10">
                  Email
                </Label>
                <Input
                  id="email"
              ...
              <Button
                type="submit"
                className="w-full mt-5 h-12 text-base flex items-center justify-center"
                disabled={isLoading}
              >
